...
while True:
    ...
    line_width = 1
    color = (123,123,123)
    img_yuv = cv2.cvtColor(np.array(img), cv2.COLOR_BGR2YUV_I420)
#     img = cv2.resize(img, dim)
#     image_np = np.array(img)
#     img_yuv = cv2.cvtColor(image_np, cv2.COLOR_BGR2YUV_I420)
    ...
    print(f'array shape -->{len(parsed_images)}\n\n')
    ...
    if (len(parsed_images) >= 2):
        ...
        print(f'parssed np array shape before -->{parsed_arr.shape}\n\n')
        parsed_arr.resize((1,12,128,256))
        np.save('data_x', parsed_arr)
        print(f'parssed np array shape after-->{parsed_arr.shape}\n\n')
        
    
    
#     points = np.array([[300, 200], [300, 220], [300, 230], [300, 250]])
    # points.dtype => 'int64'
#     cv2.polylines(image_np, np.int32([points]), 1, (255,255,255))

    
    

    cv2.imshow('window',cv2.resize(img_yuv,(512,256)))
